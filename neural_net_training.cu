...
void TrainNet(NET* Net, INT Epochs)
{
  ...
  for (n=0; n<Epochs*TRAIN_YEARS; n++) {
    ...
    SimulateNet(Net, &(Sunspots[Year-NN_YEARS]), Output, &(Sunspots[Year]), TRUE);
  }
}
...
void TestNet(NET* Net)
{
  ...
  for (Year=TRAIN_LWB; Year<=TRAIN_UPB; Year++) {
    SimulateNet(Net, &(Sunspots[Year-NN_YEARS]), Output, &(Sunspots[Year]), FALSE);
...
void EvaluateNet(NET* Net)
{
  ...
  for (Year=EVAL_LWB; Year<=EVAL_UPB; Year++) {
    SimulateNet(Net, &(Sunspots [Year-NN_YEARS]), Output,  &(Sunspots [Year]), FALSE);
    SimulateNet(Net, &(Sunspots_[Year-NN_YEARS]), Output_, &(Sunspots_[Year]), FALSE);
